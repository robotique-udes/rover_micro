...
- [Rovus micro controllers projects](#rovus-micro-controllers-projects)
  ...
  - [General guidelines](#general-guidelines)
    ...
    - [Developpement board buying guide](#developpement-board-buying-guide)
  - [None MicroRos project (prefered)](#none-microros-project-prefered)
    - [Platformio.ini](#platformioini)
      - [Platformio.ini ESP32-S2 (Developpement board)](#platformioini-esp32-s2-developpement-board)
...
- Don't use MicroRos if you can.
- Use ESP32-* for all microcontrollers projects
- Develop in VSCode with platformIO
- Run your serial ports at 115200 baud
- Limit dynamic allocation at run time and follow embedded coding guidelines as much as you can
...
### Developpement board buying guide
...
## None MicroRos project (prefered)
...
At the current time there's are only ESP32-S3 (On custom PCB) and ESP32-S2 (On developpement board). They have some slight difference in platformio.ini config but are fairely the same to work with.

### Platformio.ini

#### Platformio.ini ESP32-S2 (Developpement board)
...
- Rover_helpers
  - Includes multiple simple classes to help developement and aid with code readability and consistency (between project). It include, but is not limited to;
    - A Timer for constant* loop interval
    - A Chrono for time mesurement
    - Led Blinkers for making led pattern easily (software and hardware version available)
    - A Logger which is similar to the ROS logging mecanisms
    - Macros of all sorts (see code as documentation)
    - A Moving Average (low pass filter with constant coefficient)
    - A PID class
    - Some Assertion macros

All code which can be reused should be added to the lib and not be local to projects. If they only work for certain microcontrollers, include guards should be used to prevent usage with wrong microcontrollers. Try to always use existing helpers/drivers instead of writting your own version (improve them if necessary).

#### rover_can_lib

This library is the same as the linux x86 version. [See the documentation on the main rover repo](https://github.com/robotique-udes/rover/tree/release/circ2024/doc/can).
...
Since were running ROS2, some microcontrollers might run [microROS](https://micro.ros.org/) and all developpement will be in platformIO. This [tutorial](https://www.youtube.com/watch?v=Nf7HP9y6Ovo) is a great first step at learning microROS. Those are the steps from the tutorial you should do:
