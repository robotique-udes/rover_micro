...
public:
    ...
    CanBusManager(uint16_t deviceId_,
        ...
        : _statusLed(pinStatusLED_),
          _timerHeartbeat(static_cast<unsigned long>(1000.0f / static_cast<float>(HEARTBEAT_FREQ)))
    {
        _id = deviceId_;
...
private:
    ...
    Timer<unsigned long, millis> _timerWatchdog = Timer<unsigned long, millis>(500);
